...
export function CategoryUsageDrawer({
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent
        className="mx-auto max-h-[85dvh] max-w-lg gap-0 overflow-hidden rounded-t-[1.75rem] border-0 px-0 pb-[env(safe-area-inset-bottom)] text-white [&>div:first-child]:hidden"
        style={{ backgroundColor: activeCategory.color }}
      >
        <div className="relative px-5 pb-2 pt-6">
          <div className="absolute right-5 top-0 -translate-y-1/2 rounded-full bg-white p-1.5 shadow-sm">
            <span
            ...
            <div className="mt-4 rounded-2xl bg-black/15 px-3 py-3">
              ...
              <div className="mt-1.5 flex items-center justify-between text-xs text-white/80">
                ...
                <span className="tabular-nums">
                  {formatIdr(
                    Math.max(0, activeCategory.budgetedAmount - spentAmount),
                  )}{" "}
                  left
                </span>
              </div>
